...
const createSeeleProxy = async (
    ...
    const initData = seeleMaster.interface.encodeFunctionData("setUp", [
      encodedInitParams,
    ]);
    ...
    const salt = utils.solidityKeccak256(
      ["bytes32", "uint256"],
      [utils.solidityKeccak256(["bytes"], [initData]), "0x01"]
    );
...
export default createSeeleProxy
