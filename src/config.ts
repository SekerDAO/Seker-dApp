import {isAddress} from "@ethersproject/address"
import {urlRegex} from "./utlls"

/* eslint-disable no-process-env */
const {
	REACT_APP_FIREBASE_API_KEY,
	REACT_APP_FIREBASE_AUTH_DOMAIN,
	REACT_APP_FIREBASE_PROJECT_ID,
	REACT_APP_FIREBASE_STORAGE_BUCKET,
	REACT_APP_FIREBASE_MESSAGING_SENDER_ID,
	REACT_APP_FIREBASE_APP_ID,
	REACT_APP_CLOUD_FUNCTIONS_URL,
	REACT_APP_IPFS_ENDPOINT,
	REACT_APP_CHAIN_ID,
	REACT_APP_INFURA_NETWORK,
	REACT_APP_INFURA_ID,
	REACT_APP_DOMAIN_ADDRESS,
	REACT_APP_WETH_ADDRESS,
	REACT_APP_PROXY_ADDRESS,
	REACT_APP_AUCTION_ADDRESS,
	REACT_APP_USUL_MASTERCOPY_ADDRESS,
	REACT_APP_MODULE_FACTORY_ADDRESS,
	REACT_APP_OZ_LINEAR_MASTER_ADDRESS,
	REACT_APP_MULTI_SEND_ADDRESS
} = process.env
/* eslint-enable no-process-env */

if (!REACT_APP_FIREBASE_API_KEY) {
	throw new Error("Required env variable REACT_APP_FIREBASE_API_KEY not set")
}
if (!REACT_APP_FIREBASE_AUTH_DOMAIN) {
	throw new Error("Required env variable REACT_APP_FIREBASE_AUTH_DOMAIN not set")
}
if (!REACT_APP_FIREBASE_PROJECT_ID) {
	throw new Error("Required env variable REACT_APP_FIREBASE_PROJECT_ID not set")
}
if (!REACT_APP_FIREBASE_STORAGE_BUCKET) {
	throw new Error("Required env variable REACT_APP_FIREBASE_STORAGE_BUCKET not set")
}
if (!REACT_APP_FIREBASE_MESSAGING_SENDER_ID) {
	throw new Error("Required env variable REACT_APP_FIREBASE_MESSAGING_SENDER_ID not set")
}
if (!REACT_APP_FIREBASE_APP_ID) {
	throw new Error("Required env variable REACT_APP_FIREBASE_APP_ID not set")
}
if (!REACT_APP_CLOUD_FUNCTIONS_URL || !REACT_APP_CLOUD_FUNCTIONS_URL.match(urlRegex)) {
	throw new Error("Required env variable REACT_APP_CLOUD_FUNCTIONS_URL not set or malformed")
}

if (!REACT_APP_IPFS_ENDPOINT || !REACT_APP_IPFS_ENDPOINT.match(urlRegex)) {
	throw new Error("Required env variable REACT_APP_IPFS_ENDPOINT not set or malformed")
}
if (!REACT_APP_CHAIN_ID || isNaN(Number(REACT_APP_CHAIN_ID))) {
	throw new Error("Required env variable REACT_APP_CHAIN_ID not set or malformed")
}
if (!REACT_APP_INFURA_NETWORK) {
	throw new Error("Required env variable REACT_APP_INFURA_NETWORK not set")
}
if (!REACT_APP_INFURA_ID) {
	throw new Error("Required env variable REACT_APP_INFURA_ID not set")
}

if (!REACT_APP_DOMAIN_ADDRESS || !isAddress(REACT_APP_DOMAIN_ADDRESS)) {
	throw new Error("Required env variable REACT_APP_DOMAIN_ADDRESS not set or malformed")
}
if (!REACT_APP_WETH_ADDRESS || !isAddress(REACT_APP_WETH_ADDRESS)) {
	throw new Error("Required env variable REACT_APP_WETH_ADDRESS not set or malformed")
}
if (!REACT_APP_PROXY_ADDRESS || !isAddress(REACT_APP_PROXY_ADDRESS)) {
	throw new Error("Required env variable REACT_APP_PROXY_ADDRESS not set or malformed")
}
if (!REACT_APP_AUCTION_ADDRESS || !isAddress(REACT_APP_AUCTION_ADDRESS)) {
	throw new Error("Required env variable REACT_APP_AUCTION_ADDRESS not set or malformed")
}
if (!REACT_APP_USUL_MASTERCOPY_ADDRESS || !isAddress(REACT_APP_USUL_MASTERCOPY_ADDRESS)) {
	throw new Error("Required env variable REACT_APP_USUL_MASTERCOPY_ADDRESS not set or malformed")
}
if (!REACT_APP_MODULE_FACTORY_ADDRESS || !isAddress(REACT_APP_MODULE_FACTORY_ADDRESS)) {
	throw new Error("Required env variable REACT_APP_MODULE_FACTORY_ADDRESS not set or malformed")
}
if (!REACT_APP_OZ_LINEAR_MASTER_ADDRESS || !isAddress(REACT_APP_OZ_LINEAR_MASTER_ADDRESS)) {
	throw new Error("Required env variable REACT_APP_OZ_LINEAR_MASTER_ADDRESS not set or malformed")
}
if (!REACT_APP_MULTI_SEND_ADDRESS || !isAddress(REACT_APP_MULTI_SEND_ADDRESS)) {
	throw new Error("Required env variable REACT_APP_MULTI_SEND_ADDRESS not set or malformed")
}

const config = {
	firebase: {
		FIREBASE_API_KEY: REACT_APP_FIREBASE_API_KEY,
		FIREBASE_AUTH_DOMAIN: REACT_APP_FIREBASE_AUTH_DOMAIN,
		FIREBASE_PROJECT_ID: REACT_APP_FIREBASE_PROJECT_ID,
		FIREBASE_STORAGE_BUCKET: REACT_APP_FIREBASE_STORAGE_BUCKET,
		FIREBASE_MESSAGING_SENDER_ID: REACT_APP_FIREBASE_MESSAGING_SENDER_ID,
		FIREBASE_APP_ID: REACT_APP_FIREBASE_APP_ID
	},
	CLOUD_FUNCTIONS_URL: REACT_APP_CLOUD_FUNCTIONS_URL,
	IPFS_ENDPOINT: REACT_APP_IPFS_ENDPOINT,
	CHAIN_ID: Number(REACT_APP_CHAIN_ID),
	INFURA_NETWORK: REACT_APP_INFURA_NETWORK,
	INFURA_ID: REACT_APP_INFURA_ID,
	DOMAIN_ADDRESS: REACT_APP_DOMAIN_ADDRESS,
	WETH_ADDRESS: REACT_APP_WETH_ADDRESS,
	PROXY_ADDRESS: REACT_APP_PROXY_ADDRESS,
	AUCTION_ADDRESS: REACT_APP_AUCTION_ADDRESS,
	USUL_MASTERCOPY_ADDRESS: REACT_APP_USUL_MASTERCOPY_ADDRESS,
	MODULE_FACTORY_ADDRESS: REACT_APP_MODULE_FACTORY_ADDRESS,
	OZ_LINEAR_MASTER_ADDRESS: REACT_APP_OZ_LINEAR_MASTER_ADDRESS,
	MULTI_SEND_ADDRESS: REACT_APP_MULTI_SEND_ADDRESS
}

export default config
